...
class AgentLoop:
    ...
    def __init__(
        ...
        from core.config.schema import ExecToolConfig
        self.bus = bus
    ...
    def _register_default_tools(self) -> None:
        ...
        for cls in (ReadFileTool, WriteFileTool, EditFileTool, ListDirTool):
            kwargs: dict[str, Any] = {"workspace": self.workspace, "allowed_dir": allowed_dir}
            if cls in (WriteFileTool, EditFileTool):
                kwargs["on_write"] = self._on_file_write
            self.tools.register(cls(**kwargs))
        ...
        if self.integrations_config:
            ...
            if jira_cfg.api_token and jira_cfg.email and jira_cfg.base_url:
                from core.agent.tools.jira import (
            ...
            if notion_cfg.api_key:
                from core.agent.tools.notion import (
                    NotionAppendBlocksTool, NotionCreateDatabaseEntryTool,
                    NotionCreateDatabaseTool, NotionCreatePageTool,
                    NotionDeleteBlockTool, NotionGetPageTool,
                    NotionQueryDatabaseTool, NotionSearchTool,
                    NotionUpdateDatabaseTool, NotionUpdatePageTool,
                    _NotionClient,
                )
                notion_client = _NotionClient(notion_cfg.api_key)
                root = notion_cfg.root_page_id
                self.tools.register(NotionCreatePageTool(notion_client, root))
                self.tools.register(NotionGetPageTool(notion_client))
                self.tools.register(NotionUpdatePageTool(notion_client))
                self.tools.register(NotionDeleteBlockTool(notion_client))
                self.tools.register(NotionAppendBlocksTool(notion_client))
                self.tools.register(NotionSearchTool(notion_client))
                self.tools.register(NotionCreateDatabaseTool(notion_client, root))
                self.tools.register(NotionUpdateDatabaseTool(notion_client))
                self.tools.register(NotionCreateDatabaseEntryTool(notion_client))
                self.tools.register(NotionQueryDatabaseTool(notion_client))

    async def _on_file_write(self, file_path: Path) -> None:
        """Callback invoked after a .md file is written inside the workspace."""
        if not file_path.suffix == ".md":
            return
        if not self.agents_config or not self.agents_config.bucket:
            return
        try:
            workspace = self.workspace.expanduser().resolve()
            file_path = file_path.resolve()
            file_path.relative_to(workspace)  # raises ValueError if outside
        except ValueError:
            return
        asyncio.create_task(self._push_to_s3(file_path))

    async def _push_to_s3(self, file_path: Path) -> None:
        """Fire-and-forget background push of a single file to S3."""
        try:
            await asyncio.get_event_loop().run_in_executor(
                None,
                push_file,
                self.workspace,
                self.agents_config.bucket,
                self.agents_config.instance,
                file_path,
            )
        except Exception as e:
            logger.error("S3 push failed for {}: {}", file_path, e)
    ...
    async def _connect_mcp(self) -> None:
        ...
        from core.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    async def _consolidate_memory(self, session, archive_all: bool = False) -> bool:
        ...
        return await MemoryStore(self.workspace, on_write=self._on_file_write).consolidate(
